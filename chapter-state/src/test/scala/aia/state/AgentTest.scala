...
class AgentTest extends TestKit(ActorSystem("AgentTest"))
  ...
  "Agent" must {
    "test1" in {
      ...
      Future {
        Thread.sleep(2000)
        agent send StateBookStatistics(22, Map())
      }
      println("1: " + agent())
      atomic {
        txn ⇒
          ...
          println("2: " + agent())
          Thread.sleep(5000)
          agent send StateBookStatistics(value.sequence + 1, Map())
      }
      println("3: " + agent())
  ...
  "AgentMgr" must {
    "test" in {
      ...
      val book = BookStatistics(bookName, 2)
      mgr.getStateBookStatistics() must be(StateBookStatistics(2, Map(bookName -> book)))
    }
    "test alter" in {
      val bookName = "Akka in Action"
      val mgr = new BookStatisticsMgr(system)
      ...
      val book = BookStatistics(bookName, 2)
      state must be(StateBookStatistics(2, Map(bookName -> book)))
    }
  }
}
